...
parser.add_argument('--o', type = str, required = True, default = 'out')
...
class Detection(object):
    ...
    def inference(self, image, size):
        ort_inputs = {self.session.get_inputs()[0].name:image[None,:,:,:], self.session.get_inputs()[1].name:size}
        box_out, scores_out, classes_out = self.session.run(None, ort_inputs)
        return box_out, scores_out, classes_out
 

    def draw_line(self, image, x, y, x1, y1, color, l = 35, t = 2):
    ...
    def draw_visual(self, image, boxes_out, scores_out, classes_out, lines = True):
        ...
        for i, c in reversed(list(enumerate(classes_out))):
            ...
            cv2.putText(image, predicted_class_label, (x_min, y_min-5), cv2.FONT_HERSHEY_SIMPLEX, 0.35, [0, 255,255], 1)
        return image
    
    
    
    def predict_image(self, image, video = False):
        if not video: image = Image.open(image).convert("RGB")
        input_image_shape = np.expand_dims(np.array([image.size[1], image.size[0]], dtype='float32'), 0)       
        image_data  = self.resize_image(image, (self.image_shape[1], self.image_shape[0]))
        image_data  = self.preprocess_input(np.array(image_data, dtype='float32'))       
...
logging.basicConfig(filename=f'yolox.log', filemode='w', format='%(asctime)s - %(message)s', level=logging.INFO, datefmt='%d-%b-%y %H:%M:%S')
